...
namespace MyWeb.ServiceLayer.Contracts
{
  ...
  public interface IApplicationUserManager : IDisposable
  {
    ...
    bool IsEmailAvailableForConfirm(string email);
    ...
    Task<User> CheckRemoteAuthentication(string username, string password);

    IEnumerable<SelectListItem> GetAllUsersSelectListItemForSearch();

    Task<EditUserMainPageViewModel> GetUserForMainById(long id);
    Task<bool> EditUserMainPage(EditUserMainPageViewModel viewModel);
